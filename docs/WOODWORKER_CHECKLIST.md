# Woodworker readiness checklist (Grainline)

Use this alongside the main tabs. It describes what “ready for the shop” means in this tool—not every possible real-world step (hardware pull, finish schedule, etc.).

## Guided sequence

1. **Project** — Project name, milling allowance, max transport length, widest board you can buy (nominal / actual), waste %, workshop defaults.
2. **Plan** — Choose a preset (dresser, board pack, sideboard shell, …). Enter intent and confirm preview/proportions.
3. **Materials** — Review yard list, cut layout, and source part rows. Fix blockers in Plan if this tab is locked.

## Pass / fail (mirrors the in-app checklist)

| Item | How to satisfy |
|------|----------------|
| Named project | Non-empty name in Setup. |
| Parts exist | At least one part row (from Plan generation or manual add on Cut list). |
| Blocking issues cleared | Materials tab opens only when blocking validation issues are resolved. |
| Export / print available | Use the source parts section (CSV) or **`/print`** once the cut list reflects intent. |
| Shop status (optional) | Open **`/shop`** from **Project** for a read-only large-type summary before you walk away from the desk. |

## Not covered here (honest gaps)

Treat these as **your** shop checklist unless the app adds them later:

- Slide/hardware vendor worksheets vs clearances  
- Face frame / inset / overlay reveals  
- Finish, sanding allowances, and machine-specific sequences  
- Sheet goods optimization (focus here is hardwood-style parts + buy guidance)

For product direction, see `docs/PRODUCT_STRATEGY.md` and `docs/PRD.md`.
